...
public class CompletedSurveyFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		adapter = new ArrayAdapter<String>(context, R.layout.item_text_view,
				featureListStr);
	...
	private void refreshList() {
		...
		List<Feature> features = db.fetchCompletedFeatures();
		db.close();
		featureListStr.clear();
		String geomtype = "";
		for (int i = 0; i < features.size(); i++) {
			if (features.get(i).getGeomtype()
					.equals(CommonFunctions.GEOM_POINT)) {
				geomtype = context.getResources().getString(R.string.point_txt);
			} else if (features.get(i).getGeomtype()
					.equals(CommonFunctions.GEOM_LINE)) {
				geomtype = context.getResources().getString(R.string.line_txt);
			} else if (features.get(i).getGeomtype()
					.equals(CommonFunctions.GEOM_POLYGON)) {
				geomtype = context.getResources().getString(
						R.string.polygon_txt);
			}
	...
	public void setUserVisibleHint(boolean isVisibleToUser) {
		...
		if (isVisibleToUser && DraftSurveyFragment.listChanged) {
			refreshList();
			Toast.makeText(context, R.string.refreshing_msg, Toast.LENGTH_SHORT)
					.show();
