...
public class AttributeAdapter extends BaseAdapter 
{
	...
	private View createInputRow(View container, String attribLable,String AnsText,final int currentposition,boolean error) 
	{
		...
		if (error) 
		{
			fieldValue.setBackgroundColor(context.getResources().getColor(R.color.lightred));
		}
	...
	private View createTimePickerRow(View container, String attribLable,String AnsText,final int currentposition,boolean error) 
	{
		...
		if (error) 
		{
			textdatepicker.setBackgroundColor(context.getResources().getColor(R.color.lightred));
		}
		...
		textdatepicker.setOnClickListener(new OnClickListener() 
		{
			...
			{
				final Dialog custom_timepicker=new Dialog(context,R.style.DialogTheme);
	...
	Spinner createSpinnerViewFromArray(View container,List<Option> values,String attribLable,String selectedAns,final int position,boolean error) 
	{
		...
		boolean isnonNatural=CommonFunctions.isNonNatural(featureId);
		if(isnonNatural)
		{
			selectedAns="106";
			spinner.setEnabled(false);			
		...
		if (error) 
		{
			spinner.setBackgroundColor(context.getResources().getColor(R.color.lightred));
		}
		
		
	
			if (attribList.get(position).getAttributeid() == 23) {
