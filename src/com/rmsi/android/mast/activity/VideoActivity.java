...
public class VideoActivity extends Activity implements Callback {
	...
	private File initFile() {
		...
		// "Vaibhav"); //set folder Vaibhav
		File dir = new File(Environment.getExternalStorageDirectory()
				.getAbsolutePath()
				+ "/"
				+ CommonFunctions.parentFolderName
				+ "/" + CommonFunctions.mediaFolderName); // in Movie dir

		if (!dir.exists()) {
			Log.wtf("Video Cap",
					"Failed to create storage directory: " + dir.getPath());
			Toast.makeText(this, "not record", Toast.LENGTH_SHORT).show();
			file = null;
		} else {
			java.util.Date date = new java.util.Date();
			String timeStamp = new SimpleDateFormat("MMdd_HHmmss").format(date
					.getTime());
			file = new File(dir.getPath() + File.separator + "MTP_" + timeStamp
					+ ".mp4"); // set the file name in mp4 format
		}
		return file;
	}
	...
	private void initRecorder() {
		...
		CamcorderProfile cpVga = CamcorderProfile
				.get(CamcorderProfile.QUALITY_QVGA);// Res 320*240;
		recorder.setProfile(cpVga);
		recorder.setOutputFile(this.initFile().getPath()); // initFile is the
															// name of the file
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		DisplayMetrics displaymetrics = new DisplayMetrics(); // DisplayMetrics
																// to set height
																// of surface
																// view
		getWindowManager().getDefaultDisplay().getMetrics(displaymetrics); // according
																			// to
																			// device.
			...
			public void onClick(View v) {
				if (recording) {
					...
					try {
						...
						e.printStackTrace();
						Log.i("Database Problem",
								"Problem in data insert operation");
					}
