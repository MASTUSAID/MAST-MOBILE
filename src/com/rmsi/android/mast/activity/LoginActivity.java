...
public class LoginActivity extends ActionBarActivity {
	private class validateUser extends AsyncTask<User, Integer, String> {
		...
		protected String doInBackground(User... user) {
			...
			try {
				...
				String urlParameters = "email=" + userName + "&password="
						+ password;
			...
			try {
				if (!TextUtils.isEmpty(response)) {
					...
					if (Obj.equals(null)) {
						...
						error_msg = getResources().getString(
						...
						if (Obj.has("email")) {
							values.put("USER_NAME", Obj.get("name").toString());
						}
						if (Obj.has("password")) {
							values.put("PASSWORD", Obj.get("password")
									.toString());
						}
						if (Obj.has("roles")) {
							JSONArray jsonarrayForRoles = Obj
									.getJSONArray("roles");
							childObj = new JSONObject(jsonarrayForRoles.get(0)
									.toString());
							role = childObj.get("name").toString();
							values.put("ROLE_ID", childObj.get("id").toString());
							values.put("ROLE_NAME", childObj.get("name")
									.toString());
						}
						valueList.add(values);
						String tableName = "USER";
						loginAction(true, "Success");
						new DBController(getApplicationContext()).InsertValues(
								valueList, tableName);
					}
				} else {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * DBController sqllite = new DBController(context);
		 * sqllite.onCreate(sqllite.getWritableDatabase()); boolean isdb =
		 * sqllite.checkDataBase(); sqllite.close();
		 * 
		 * if(isdb) { Toast.makeText(this, "DATABASE Created",
		 * Toast.LENGTH_SHORT).show(); } else { Toast.makeText(this,
		 * "DATABASE Creation failed", Toast.LENGTH_SHORT).show(); }
		 */
		// Setting toolbar
		/*
	...
	private void validateUserOnline(User newUser) {
		if (cf.getConnectivityStatus()) {
			ringProgressDialog = ProgressDialog.show(new ContextThemeWrapper(
					context, android.R.style.Theme_Holo), null, getResources()
					.getString(R.string.server_logging_msg), true);
