...
public class LandingPageActivity extends ActionBarActivity implements Receiver {
	...
	private boolean exportDB() {
		String timeStamp = new SimpleDateFormat("MMdd_HHmmss", Locale.US)
				.format(new Date().getTime());
		String Ext_Storage_Path = Environment.getExternalStorageDirectory()
				.getAbsolutePath();

		String currDBPATH = "/" + CommonFunctions.parentFolderName + "/"
				+ CommonFunctions.dbFolderName + "/MAST_mobile.db";
		String backupDBPATH = "/" + CommonFunctions.parentFolderName + "/"
				+ CommonFunctions.dbFolderName + "/Backup_MAST_mobile_"
				+ timeStamp + ".db";

		// String currentDBPath = "/data/"+ "com.your.package"
		// +"/databases/"+db_name;
		...
		try {
			...
			if (success)
				cf.addErrorMessage("LOGOUT >> ", "DB Exported!!"); // logging
																	// logout
																	// time
			else
				return false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (roleId == 1) // Hardcoded Id for Role (1=Trusted Intermediary,
							// 2=Adjudicator)
		{
		...
		btn_exit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				DBController sqllite = new DBController(context);
				boolean pendingRecords = sqllite
						.checkPendingDraftAndCompletedRecordsToSync();
				sqllite.close();
				if (!pendingRecords) {
							...
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface arg0,
									...
									if (isDatabaseBackedup) {
										// OLD logic of removing user & project
										// details.
										/*
										 * boolean result_user = new
										 * DBController
										 * (context).removeLoggedUser();
										 * if(result_user) {
										 */
					...
					String syncMsg = getResources().getString(
							R.string.sync_pending_records);
			...
			public void onClick(View arg0) {
				if (cf.getConnectivityStatus()) {
					...
					if (user != null && user.getUserId() != null) {
						String connectionMsg = getResources().getString(
								R.string.webserviceconnectMsg);
	...
	public void onReceiveResult(int resultCode, Bundle resultData) {
		...
		String DataDownloadCompleted = getResources().getString(
				R.string.DataDownloadCompleted);
		String ErrorInDownloadingData = getResources().getString(
				R.string.ErrorInDownloadingData);
	 ...
	 * case DISPLAY_DLG: if (!isFinishing()) { showDialog(MY_DIALOG); } break; }
	 * } };
	 */
