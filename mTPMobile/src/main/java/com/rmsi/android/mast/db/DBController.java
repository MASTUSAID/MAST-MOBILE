...
public class DBController extends SQLiteOpenHelper {
	...
	private static String CAT_POI = "8";
	...
	public DBController(Context applicationcontext) {
		super(applicationcontext, DB_FULL_PATH, null, 1);
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String query_table1 = "CREATE TABLE SPATIAL_FEATURES (FEATURE_ID INTEGER PRIMARY KEY AUTOINCREMENT,SERVER_FEATURE_ID TEXT,COORDINATES TEXT,GEOMTYPE TEXT,CREATEDTIME TEXT,STATUS TEXT,COMPLETEDTIME TEXT,SERVER_PK TEXT,IMEI TEXT,PARCEL_TYPE TEXT,HAMLET_ID INTEGER,OTHER_EXISTING_USE TEXT)";
		String query_table2 = "CREATE TABLE ATTRIBUTE_MASTER(ID INTEGER PRIMARY KEY AUTOINCREMENT,ATTRIB_ID INTEGER,ATTRIBUTE_TYPE STRING,ATTRIBUTE_CONTROLTYPE INTEGER,ATTRIBUTE_NAME TEXT,LISTING TEXT,ATTRIBUTE_NAME_OTHER TEXT,VALIDATION TEXT,IS_EXIST TEXT)";
		...
		String query_table10 = "CREATE TABLE SOCIAL_TENURE(ID INTEGER PRIMARY KEY,TENURE_TYPE_ID INTEGER,PERSON_ID INTEGER,FEATURE_ID TEXT,SERVER_PK TEXT)";
		...
		String query_table13 = "CREATE TABLE PROJECT_SPATIAL_DATA(SERVER_PK INTEGER,PROJECT_NAME TEXT,FILE_NAME TEXT,FILE_EXT TEXT,ALIAS TEXT,VILLAGE_NAME TEXT,COMMUNE TEXT)";
		...
		String query_table18 = "CREATE TABLE COMMUNE(COMMUNE_ID INTEGER PRIMARY KEY,COMMUNE_NAME INTEGER,PROVINCE_ID INTEGER TEXT)";
		String query_table19 = "CREATE TABLE Village(VILLAGE_ID TEXT,ATTRIB_ID INTEGER,VILLAGE_NAME TEXT,VILLAGE_NAME_OTHER TEXT)";
		String query_table20 = "CREATE TABLE POI(ID INTEGER PRIMARY KEY,ATTRIB_1 TEXT,ATTRIB_2 TEXT,FEATURE_ID TEXT)";
		try {
...
//			db.execSQL(query_table16);
	...
	public List<Feature> fetchDraftFeatures() {
		...
		String q = "SELECT sf.*,P.* FROM SPATIAL_FEATURES as sf left join PERSON as P  on sf.FEATURE_ID=P.FEATURE_ID  where sf.status = '"
				+ STATUS_DRAFT + "'";
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					feature.setGeomtype(cursor.getString(3));
					feature.setStatus(cursor.getString(5));
					feature.setOwner_name(cursor.getString(13)+" "+cursor.getString(14));
	...
	public List<Feature> fetchSyncededFeatures() {
		...
		String syncedFeatureSql = "SELECT sf.*,P.* FROM SPATIAL_FEATURES as sf left join PERSON as P  on sf.FEATURE_ID=P.FEATURE_ID  where sf.status ='"
				+ STATUS_COMPLETE
				+ "' and (SERVER_FEATURE_ID IS not NULL OR SERVER_FEATURE_ID != '')";
	...
	public List<Attribute> getPersonAttribute(SQLiteDatabase database,
			String lang,String PersonFormType) {

		String selectQueryValues="";
		List<Attribute> attribList = new ArrayList<Attribute>();
		if(PersonFormType.equalsIgnoreCase("PERSON"))  
		{
			//iNDIVIDUAL
		selectQueryValues = "SELECT  * FROM ATTRIBUTE_MASTER where ATTRIBUTE_TYPE ='"
				+ CAT_NaturalPerson +"' AND IS_EXIST IN ('A','IC')";// ORDER BY ATTRIB_ID";
		...
		//EXISTING
		/*selectQueryValues = "SELECT  * FROM ATTRIBUTE_MASTER where ATTRIBUTE_TYPE ='"
				+ CAT_NaturalPerson +"' AND IS_EXIST IN ('A','E')";// ORDER BY ATTRIB_ID";
*/		}
		
		else if(PersonFormType.equalsIgnoreCase("COLLECTIVE"))
		{
			attribList = new ArrayList<Attribute>();
			selectQueryValues = "SELECT  * FROM ATTRIBUTE_MASTER where ATTRIBUTE_TYPE ='"
					+ CAT_NaturalPerson +"' AND IS_EXIST IN ('A','IC','C')";// ORDER BY ATTRIB_ID";
			
		}
		
		else if(PersonFormType.equalsIgnoreCase("POI"))
		{
			attribList = new ArrayList<Attribute>();
			selectQueryValues = "SELECT  * FROM ATTRIBUTE_MASTER where ATTRIBUTE_TYPE ='"
					+ CAT_POI + "'";// ORDER BY ATTRIB_ID";
			
		}
		else if(PersonFormType.equalsIgnoreCase("EXISTING"))
		{
			attribList = new ArrayList<Attribute>();
			selectQueryValues = "SELECT  * FROM ATTRIBUTE_MASTER where ATTRIBUTE_TYPE ='"
					+ CAT_NaturalPerson +"' AND IS_EXIST IN ('A','E')";// ORDER BY ATTRIB_ID";			
			...
			do {
				...
				if (attrib.getControlType() == 5) {
					...
					String selectQueryOptions="";	
						...
						if(attrib.getAttributeid()==282)
						{
						selectQueryOptions = "SELECT  * FROM Village where ATTRIB_ID ="+ attrib.getAttributeid();
						}
						else{
						selectQueryOptions = "SELECT  * FROM OPTIONS where ATTRIB_ID ="+ attrib.getAttributeid();
						}
	...
	public List<Attribute> getFormDataByGroupId(int GroupId, String lang,String PersonFormType) {
		SQLiteDatabase database = getReadableDatabase();
		List<Attribute> attribList = new ArrayList<Attribute>();
...
*/		
		String selectQueryValues = null;
		
		if(PersonFormType.equalsIgnoreCase("PERSON"))  
		{
			//iNDIVIDUAL
		selectQueryValues = "SELECT  AM.*,FM.GROUP_ID,FM.ATTRIB_VALUE FROM ATTRIBUTE_MASTER AM LEFT OUTER JOIN"
				+ " (SELECT * FROM FORM_VALUES WHERE GROUP_ID= "+GroupId+") FM ON AM.ATTRIB_ID =  FM.ATTRIB_ID "
						+ "where AM.ATTRIBUTE_TYPE =" + CAT_NaturalPerson + " AND IS_EXIST IN ('A','IC')";
		...
		else if(PersonFormType.equalsIgnoreCase("COLLECTIVE"))
		{
			...
			selectQueryValues ="SELECT  AM.*,FM.GROUP_ID,FM.ATTRIB_VALUE FROM ATTRIBUTE_MASTER AM LEFT OUTER JOIN "
					+ "(SELECT * FROM FORM_VALUES WHERE GROUP_ID= "+GroupId+") FM ON AM.ATTRIB_ID =  FM.ATTRIB_ID"
							+ " where AM.ATTRIBUTE_TYPE =" + CAT_NaturalPerson + " AND IS_EXIST IN ('A','IC','C')";
		...
		else if(PersonFormType.equalsIgnoreCase("POI"))
		{
			...
			selectQueryValues = "SELECT  AM.*,FM.GROUP_ID,FM.ATTRIB_VALUE FROM ATTRIBUTE_MASTER AM LEFT OUTER JOIN"
					+ " (SELECT * FROM FORM_VALUES WHERE GROUP_ID= "+GroupId+") FM ON AM.ATTRIB_ID =  FM.ATTRIB_ID"
							+ " where AM.ATTRIBUTE_TYPE =" + CAT_POI + " AND IS_EXIST IN ('A','IC','C')";
		...
		else if(PersonFormType.equalsIgnoreCase("EXISTING"))
		{
			...
			selectQueryValues ="SELECT  AM.*,FM.GROUP_ID,FM.ATTRIB_VALUE FROM ATTRIBUTE_MASTER AM LEFT OUTER JOIN "
					+ "(SELECT * FROM FORM_VALUES WHERE GROUP_ID= "+GroupId+") FM ON AM.ATTRIB_ID =  FM.ATTRIB_ID "
							+ "where AM.ATTRIBUTE_TYPE =" + CAT_NaturalPerson + " AND IS_EXIST IN ('A','E')";
							}
		...
		if (cursor.moveToFirst()) {
			try {
				do {
					...
					attrib.setControlType(cursor.getInt(3));
					attrib.setValidation(cursor.getString(7));
					attrib.setFieldValue(cursor.getString(10));
					attrib.setListing(cursor.getInt(5));
					if (attrib.getControlType() == 5) {
						List<Option> optionList = new ArrayList<Option>();
	...
	public List<Attribute> getFeatureGenaralInfo(Long featureId,
		...
		if (cursor.moveToFirst()) {
			do {
				...
				attrib.setValidation(cursor.getString(7));
				attrib.setGroupId(cursor.getInt(9));
				attrib.setFieldValue(cursor.getString(10));
				//attrib.setParcelType(cursor.getString(11));
				/*attrib.setHamletId(cursor.getInt(12));
				attrib.setWitness1(cursor.getString(13));
				attrib.setWitness2(cursor.getString(14));
*/
				if (attrib.getControlType() == 5) {
				...
				{	
					...
					if (cursor2.moveToFirst()) {
						do {
							...
							option.setOptionId(cursor2.getLong(0));
							StringBuffer multiLnagText = new StringBuffer();
							if (lang.equalsIgnoreCase("fr") && !TextUtils.isEmpty(cursor2.getString(3))) {
								option.setOptionName(cursor2.getString(3));
							} else {
								option.setOptionName(cursor2.getString(2));
							}
	...
	public boolean saveFormDataTemp(List<Attribute> attribList, int groupid,
		...
		if (groupid != 0) {
			try {
				...
				db.delete("POI", deleteDataWhereID, null);
					...
					if (attribute.getFieldValue() != null) {
						...
						if (attribute.getListing() == 2)
							tableValues.put("ATTRIB_2",
									attribute.getFieldValue());
/*
						if (attribute.getAttributeid() == 274) {
							String selectQueryOptions = "SELECT OPTION_NAME from OPTIONS WHERE OPTION_ID ="
									+ attribute.getFieldValue();
							Cursor cursor = db.rawQuery(selectQueryOptions,
									null);
							if (cursor.moveToFirst()) {
								do {
									ContentValues valuesForParcelType = new ContentValues();
									valuesForParcelType.put("PERSON_TYPE",cursor.getString(0));
									db.update("SPATIAL_FEATURES",valuesForParcelType,"FEATURE_ID = " + featureId, null);
				...
				if (keyword.equalsIgnoreCase("POI")) {
					db.insert("POI", null, tableValues);
				}
		...
		if (cursor.moveToFirst()) {
			do {
				...
				if (!TextUtils.isEmpty(personSubType)) {
					String lang = "";
					lang = cf.getLocale();
					if (personSubType.equalsIgnoreCase("Owner")) // TODO check for language will b added.
					{
	...
	public boolean saveProjectData(String data) {
		...
		try {
			...
			database.delete("VILLAGE", null, null);
			if (projectdata.has("SpatialData")) {
				...
				if (project_info.length() > 0) {
					for (int i = 0; i < project_info.length(); i++) {
						...
						if (projectdata.has("ProjectInfo")) {

							
							JSONObject infoProject = projectdata.getJSONObject("ProjectInfo");
							//JSONObject infoProject = new JSONObject(projectInFo.get(0).toString());
							projectValues.put("COMMUNE",infoProject.getString("commune"));
							projectValues.put("PROJECT_NAME",infoProject.getString("name"));
							String latStr=infoProject.getString("lattitude");
							String lngStr=infoProject.getString("longitude");
							
							cf.saveLatLong(latStr, lngStr);	
			...
			/*if(projectdata.has("lattitude") && projectdata.has("longitude"))
			{
			double lat=projectdata.getDouble("lattitude");
			double lng=projectdata.getDouble("longitude");
			...
			if(projectdata.has("VillageId"))
			{
			
			JSONArray attributeOptions = projectdata
					.getJSONArray("VillageId");
			if (attributeOptions.length() > 0) {
				for (int j = 0; j < attributeOptions.length(); j++) {
					...
					option_value.put("VILLAGE_ID",optionValues.getString("villageId"));
					option_value.put("ATTRIB_ID",282);
					option_value.put("VILLAGE_NAME",optionValues.getString("villageName"));
					option_value.put("VILLAGE_NAME_OTHER",optionValues.getString("villageNameFr"));
					database.insert("VILLAGE", null,option_value);
					option_value.clear();
				}
			}
			...
			/*
			 * if(projectdata.has("ProjectExtent")){
			 * 
			 * JSONObject minExtent=new
			 * JSONObject(projectdata.getString("MinExtent")); String
			 * min_Extent=minExtent.toString(); cf.saveMinExtent(min_Extent);
			 * 
			 * }
			 */
/*
			if (projectdata.has("Hamlet")) {
				...
				if (project_info.length() > 0) {
					for (int i = 0; i < project_info.length(); i++) {
						...
						projectValues.put("HAMLET_NAME",
								project_detail.getString("hamletName"));
			 ...
			 * 
			 * 
			 * //JSONObject attribute_detail=new JSONObject(); String
			 * villageName=projectdata.getString("Village");
			...
			if (projectdata.has("Adjudicator")) {
				...
				JSONArray project_info = projectdata
						.getJSONArray("Adjudicator");
				if (project_info.length() > 0) {
					for (int i = 0; i < project_info.length(); i++) {
						JSONObject project_detail = new JSONObject(project_info
								.get(i).toString());
						projectValues.put("ID", project_detail.getInt("id"));
						projectValues.put("ADJUDICATOR_NAME",
								project_detail.getString("adjudicatorName"));

						database.insert("ADJUDICATOR_DETAILS", null,
								projectValues);
					}
				}
			}
				...
				if (attribute_info.length() > 0) {
					for (int i = 0; i < attribute_info.length(); i++) {
						...
						if (attribute_detail.has("datatypeIdBean")) {
							...
							if (datatypeIdBean.getInt("datatypeId") == 6) {
								if (!attribute_detail.getString("attributeOptions").equalsIgnoreCase("null")) {
						...
						if (attribute_detail.has("istype") && !TextUtils.isEmpty(attribute_detail.getString("istype")) && !attribute_detail.getString("istype")
										.equalsIgnoreCase("null")) {
							attributeValues.put("IS_EXIST",attribute_detail.getString("istype"));
						}
	...
	public String getCommune() {
		String commune = "";
		myDataBase = getReadableDatabase();
		String q = "select commune from PROJECT_SPATIAL_DATA LIMIT 1";
		Cursor cursor = myDataBase.rawQuery(q, null);

		if (cursor.moveToFirst()) {
			commune = cursor.getString(0);
			cursor.close();
		}
		return commune;
	}
	...
	/*
	 * public JSONArray getpersonSubTypeData(long featureId) { String
	 * query="SELECT * FROM FORM_VALUES WHERE ATTRIB_ID=54 AND FEATURE_ID="
	 * +featureId; SQLiteDatabase db=getReadableDatabase(); Cursor
	 ...
	 * if(cursor.moveToFirst()) {
	 * 
	 * try { do{ JSONArray personSubTypeJsonArr=new JSONArray();
	 ...
	 * 
	 * personSubTypeJsonList.put(personSubTypeJsonArr); } while
	 * (cursor.moveToNext()); cursor.close(); } catch (Exception e) {
	 * 
	 * }
	 * 
	 * }
	...
	public JSONArray getpersonSubTypeValue(long featureId, int groupId) {
		String query = "SELECT * FROM FORM_VALUES WHERE ATTRIB_ID=54 AND GROUP_ID="
				+ groupId + " AND FEATURE_ID=" + featureId;
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					//change by vaibhav
					spatialFeatures.put(6, cursor.getString(9));// PersonType change to parcel type
					spatialFeatures.put(7, projName);// ProjectName
					spatialFeatures.put(8, user.getUserId());// user Name
					spatialFeatures.put(9, cursor.getString(11));// OtherExistingUse
...
*/
					...
					attribValuesArr.put(0, atribsArrList); // putting general
															// and property
															// attrib at 0
					...
					/*
					 * //getting person sub type data by feature Id & hardcoded
					 * attribute Id JSONArray personSubTypeDataArr =
					 * getpersonSubTypeData(featureId); for (int i = 0; i <
					 * personSubTypeDataArr.length(); i++) {
					...
					attribValuesArr.put(2, atribsArrList); // putting tenure at
															// 2
		
					
					//fetching POI
					atribsArrList = new JSONArray();
					JSONArray atribsArrtmp = new JSONArray();
					final_sql = attributeFeatchSql.replace("<feat_id>",
							featureId + "").replace("<category>", CAT_POI);
					cursor_attrib = myDataBase.rawQuery(final_sql, null);
					String prevGroupId = "";
					if (cursor_attrib.moveToFirst()) {
						do {
							...
							if(prevGroupId=="" || !prevGroupId.equalsIgnoreCase(cursor_attrib.getString(1))){
								prevGroupId=cursor_attrib.getString(1);
								if(atribsArrList!=null && atribsArrList.length()!=0){
									atribsArrtmp.put(atribsArrList);
									atribsArrList=new JSONArray();
								}
					...
					if(atribsArrList!=null && atribsArrList.length()!=0)
					atribsArrtmp.put(atribsArrList);
					attribValuesArr.put(3, atribsArrtmp); //POI set
				...
				while (iterator.hasNext()) {
					try {
						...
						if (spatialAttribDetailsJsonObj.has("hamletId")) {
							spatialFeatureValues.put("HAMLET_ID",
									spatialAttribDetailsJsonObj
											.getString("hamletId"));
						}
						if (spatialAttribDetailsJsonObj.has("witness1")) {
							spatialFeatureValues.put("WITNESS_1",
									spatialAttribDetailsJsonObj
											.getString("witness1"));
						}
						if (spatialAttribDetailsJsonObj.has("witness2")) {
							spatialFeatureValues.put("WITNESS_2",
									spatialAttribDetailsJsonObj
											.getString("witness2"));
						}
						 ...
						 * 
						 * JSONArray attribDetailsArr = new
						 * JSONArray(spatialDetailsArr1.get(1).toString());
						 * if(attribDetailsArr!=null) { groupId =
						 * getNewGroupId(); for(int
						 * i=0;i<attribDetailsArr.length();i++) { JSONArray
						 * attribValuesArr = new
						 * JSONArray(attribDetailsArr.get(i).toString());
						 * attribValues.clear();
						 * attribValues.put("GROUP_ID",groupId);
						 * attribValues.put
						 * ("ATTRIB_ID",attribValuesArr.getInt(0));
						 * attribValues.
						 * put("ATTRIB_VALUE",attribValuesArr.getString(1));
						...
						JSONArray attribDetailsArr = new JSONArray(
								spatialDetailsArr1.get(2).toString());
						...
						JSONArray attribSocialTenureDetailsArr = new JSONArray(
								arry.get(1).toString());

						groupId = getNewGroupId();
						// same multiple tenure is coming from downloaded data
						// ,so we will save tenure data of 0 index only.
						JSONArray attribTenureChildArr = new JSONArray(
								attribSocialTenureDetailsArr.get(0).toString());

						for (int j = 0; j < attribTenureChildArr.length(); j++) {
							JSONArray attribChildValuesArr = new JSONArray(
									attribTenureChildArr.get(j).toString());
							attribValues.clear();
							attribValues.put("GROUP_ID", groupId);
							attribValues.put("ATTRIB_ID",
									attribChildValuesArr.getInt(0)); // attribute
																		// id
							attribValues.put("ATTRIB_VALUE",
									attribChildValuesArr.getString(1)); // attribute
																		// value
							attribValues.put("FEATURE_ID", featureId);
							...
							if (attribChildValuesArr.getInt(0) == 23) {
								...
								if (attribChildValuesArr.getString(1)
										.equalsIgnoreCase("29")) {
									residentValue = "Yes";
								} else if (attribChildValuesArr.getString(1)
										.equalsIgnoreCase("29")) {
									residentValue = "No";
								}
						...
						for (int i = 0; i < attribNaturalPersonDetailsArr
							...
							for (int j = 1; j < attribChildArr.length(); j++) {
								...
								JSONArray attribChildValuesArr = new JSONArray(
										attribChildArr.get(j - 1).toString());
								attribValues.clear();
							...
							if ((attribChildArr.getString(attribChildArr
									.length() - 1)).equalsIgnoreCase("3")) {
								personSubType = "Owner";
							} else if ((attribChildArr.getString(attribChildArr
									.length() - 1)).equalsIgnoreCase("4")) {
								personSubType = "Administrator";
							} else if ((attribChildArr.getString(attribChildArr
									.length() - 1)).equalsIgnoreCase("5")) {
						...
						if (attribNonNaturalPersonDetailsArr.length() > 0) {
							...
							for (int i = 0; i < attribNonNaturalPersonDetailsArr
								...
								// insertSocialTenure(i, personId, arry,
								// featureId);
							}
						...
						JSONArray attribPOIDetailsArr = new JSONArray(arry.get(
								5).toString());
						if (attribPOIDetailsArr.length() > 0) {
							for (int i = 0; i < attribPOIDetailsArr.length(); i++) {
								JSONObject POIDetailsJsonObj = new JSONObject(
										attribPOIDetailsArr.get(i).toString());
								if (POIDetailsJsonObj.has("personName")) {
									attribValues.clear();

									attribValues.put("NEXT_KIN_NAME",
											POIDetailsJsonObj
													.getString("personName"));
									attribValues.put("FEATURE_ID", featureId);
									db.insert("NEXT_KIN_DETAILS", null,
											attribValues);
								}
							}
						}
						...
						JSONArray attribDeceasedPersonDetailsArr = new JSONArray(
								arry.get(6).toString());
						if (attribDeceasedPersonDetailsArr.length() > 0) {
							for (int i = 0; i < attribDeceasedPersonDetailsArr
									.length(); i++) {
								JSONObject DeceasedPersonDetailsJsonObj = new JSONObject(
										attribDeceasedPersonDetailsArr.get(i)
												.toString());
								attribValues.clear();
								if (DeceasedPersonDetailsJsonObj
										.has("firstname")) {
									attribValues.put("FIRST_NAME",
											DeceasedPersonDetailsJsonObj
													.getString("firstname"));
								}
								if (DeceasedPersonDetailsJsonObj
										.has("middlename")) {
									attribValues.put("MIDDLE_NAME",
											DeceasedPersonDetailsJsonObj
													.getString("middlename"));
								}
								if (DeceasedPersonDetailsJsonObj
										.has("lastname")) {
									attribValues.put("LAST_NAME",
											DeceasedPersonDetailsJsonObj
													.getString("lastname"));
								}
	...
	public String getOptionText(String optionID) {
		...
		if (cursor.moveToFirst()) {
			if (!TextUtils.isEmpty(cursor.getString(1))) {
				...
				if (lang.equalsIgnoreCase("fr"))
					multiLnagText.append(cursor.getString(1));// option name in
																// French
				else
	...
	public int getPersonMediaCount(long featureId) // count for owner and
													// guardian
	{
		int count = 0;
		SQLiteDatabase database = getReadableDatabase();

		// String selectQueryQues
		// ="SELECT PER_MEDIA.* FROM PERSON_MEDIA AS PER_MEDIA LEFT OUTER JOIN PERSON AS PER ON PER_MEDIA.PERSON_ID=PER.ID where PER.FEATURE_ID= "+featureId+" AND (PER.PERSON_SUBTYPE= 'Owner' OR PER.PERSON_SUBTYPE= 'Guardian')";
	...
	public boolean addPersontype_Hemlet_witness(long featureId,
			String personType, String hemletId, String witness_1,
			String witness_2) {
		try {
			SQLiteDatabase db = getWritableDatabase();
			ContentValues values = new ContentValues();
			...
			db.update("SPATIAL_FEATURES", values, "FEATURE_ID = " + featureId,
					null);
			return true;
		} catch (Exception e) {
	...
	public boolean add_parcelType(long featureId, String parcelType,
			String villageId) {
		try {
			SQLiteDatabase db = getWritableDatabase();
			ContentValues values = new ContentValues();
			values.put("PERSON_TYPE", parcelType);
			values.put("HAMLET_ID", villageId);
	...
	public List<Attribute> getNextOfKinList(Long featureId) {
		...
		SQLiteDatabase db = getWritableDatabase();
		String selectQueryOptions = "SELECT * from POI where FEATURE_ID="
				+ featureId;
		Cursor cursor = db.rawQuery(selectQueryOptions, null);
		if (cursor.moveToFirst()) {
			do {
	...
	public boolean deleteNextOfKin(int Id) {
		...
		SQLiteDatabase db = getWritableDatabase();
		db.delete("POI", whereClause, null);
		db.close();
		return true;
	...
	public boolean IsTenureValue(long featureId) {
		...
		SQLiteDatabase database = getReadableDatabase();
		String spatialFeatureSql = "SELECT * FROM FORM_VALUES WHERE ATTRIB_ID = (SELECT ATTRIB_ID from ATTRIBUTE_MASTER WHERE ATTRIBUTE_TYPE='"
				+ CAT_Tenure + "') and FEATURE_ID =" + featureId;
		Cursor cursor = database.rawQuery(spatialFeatureSql, null);
		if (cursor.moveToFirst()) {
			flag = true;
		}
	...
	public boolean isGuardianExist(long featureId) {
		boolean flag = false;
		SQLiteDatabase database = getReadableDatabase();
		String selectSQL = "SELECT PERSON_SUBTYPE FROM PERSON WHERE PERSON_SUBTYPE='Guardian' AND FEATURE_ID ="
				+ featureId;
		Cursor cursor = database.rawQuery(selectSQL, null);
	...
	public boolean isOwnerMinor(long featureId) {
		...
		// ****Note*****
		// This query returns Person's Group ID,Person first name,Last
		// name,feature id,Server_pk(personSubType),Person's age
		// select per.*,fv.ATTRIB_VALUE from person as per left join foRM_VALUES
		// as FV on FV.GROUP_ID=PER.ID and per.FEATURE_ID=1 where
		// PER.SERVER_PK='Owner' and fv.ATTRIB_ID=21

		// HERE ATTRIB_ID=21 (FOR AGE)

		String selectSQL = "SELECT PER.*,FV.ATTRIB_VALUE FROM PERSON AS PER LEFT JOIN FORM_VALUES AS FV ON FV.GROUP_ID=PER.ID AND PER.FEATURE_ID = "
				+ featureId
				+ " WHERE PER.PERSON_SUBTYPE='Owner' AND FV.ATTRIB_ID=21";
	...
	public boolean isAdminExist(long featureId) {
		...
		String selectSql = "SELECT * FROM PERSON WHERE PERSON_SUBTYPE='Administrator' AND FEATURE_ID ="
				+ featureId;
		Cursor cursor = database.rawQuery(selectSql, null);
		if (cursor.moveToFirst()) {
			flag = true;
	...
	public boolean isOwnerExist(long featureId) {
		...
		String selectSql = "SELECT * FROM PERSON WHERE PERSON_SUBTYPE='Owner' AND FEATURE_ID ="
				+ featureId;
		Cursor cursor = database.rawQuery(selectSql, null);
		if (cursor.moveToFirst()) {
			flag = true;
	...
	public String getPersonSubType(int groupId) {
		...
		SQLiteDatabase database = getReadableDatabase();
		String selectSQL = "SELECT PERSON_SUBTYPE FROM PERSON WHERE ID ="
				+ groupId;
		Cursor cursor = database.rawQuery(selectSQL, null);
		if (cursor.moveToFirst()) {
	...
	public boolean isPersonOfInterestExist(long featureId) {
		...
		boolean flag = false;
		SQLiteDatabase database = getReadableDatabase();
		String selectSQL = "SELECT * FROM POI WHERE FEATURE_ID ="
				+ featureId;
		Cursor cursor = database.rawQuery(selectSQL, null);
		if (cursor.moveToFirst()) {
			flag = true;
	...
	public boolean isGuradianMinor(long featureId) // same method will be used
													// to check whether owner
													// exist or not
	{
		SQLiteDatabase database = getReadableDatabase();
		boolean flag = false;
		...
		String selectSQL = "SELECT PER.*,FV.ATTRIB_VALUE FROM PERSON AS PER LEFT JOIN FORM_VALUES AS FV ON FV.GROUP_ID=PER.ID AND PER.FEATURE_ID = "
				+ featureId
				+ " WHERE PER.PERSON_SUBTYPE='Guardian' AND FV.ATTRIB_ID=21";
	...
	public String getPOIforEditing(int groupID_POI) {
		...
		SQLiteDatabase db = getReadableDatabase();
		String selectQuery = "SELECT NEXT_KIN_NAME FROM POI WHERE ID = "
				+ groupID_POI;
	...
	public String getDPforEditing(int groupID_POI) { // deceased person
		...
		SQLiteDatabase db = getReadableDatabase();
		String selectQuery = "SELECT * FROM DECEASED_PERSON WHERE ID = "
				+ groupID_POI;
		Cursor cursor = db.rawQuery(selectQuery, null);
		String dp_name = "";
		if (cursor.moveToFirst()) {

			dp_name = cursor.getString(1) + " " + cursor.getString(2) + " "
					+ cursor.getString(3);
	...
	public boolean editDeceasedPerson(String firstName, String middleName,
			String lastName, int groupID_POI) {
	...
	public boolean savePersonSubType(int GroupId, String personSubType,
			String OptionValue, long featureID) {
	...
	public Option getTenureTypeOptionsValue(long featureID) {
		...
		Option option = new Option();
		String selectQueryOptions = "SELECT OPT.OPTION_NAME,OPT.OPTION_NAME_OTHER,FM.ATTRIB_VALUE FROM OPTIONS AS OPT LEFT JOIN FORM_VALUES AS FM ON OPT.OPTION_ID=FM.ATTRIB_VALUE WHERE FM.ATTRIB_ID=31 AND FM.FEATURE_ID ="
				+ featureID; // attribute id 13=Right of accupancy type
		Cursor cursor = db.rawQuery(selectQueryOptions, null);
		if (cursor.moveToFirst()) {
			...
			String lang = cf.getLocale();
			if (lang.equalsIgnoreCase("fr"))
				option.setOptionName(cursor.getString(1)); // option name in
															// swahili
			else
	...
	public boolean saveDeceasedPerson(String firstName, String middleName,
			String lastName, Long featureId) {
	...
	public List<Attribute> getDeceasedPersonList(Long featureId) {
		...
		String selectQueryOptions = "SELECT * from DECEASED_PERSON where FEATURE_ID="
				+ featureId;
		Cursor cursor = db.rawQuery(selectQueryOptions, null);
		if (cursor.moveToFirst()) {
			do {
				Attribute attrib = new Attribute();
				String deceasedPerson = cursor.getString(1) + " "
	...
	/*
	 * public List<Attribute> getGenaralAttributeInfo(Long featureId,String
	 * keyword,String lang) { String attributeType = null;
	 * 
	 * String sql=
	 * "SELECT  AM.*,FM.GROUP_ID,FM.ATTRIB_VALUE,SF.PERSON_TYPE,SF.HAMLET_ID,SF.WITNESS_1,SF.WITNESS_2 FROM ATTRIBUTE_MASTER AS AM"
	 * +
	 * " LEFT OUTER JOIN FORM_VALUES AS FM ON AM.ATTRIB_ID =  FM.ATTRIB_ID and FM.FEATURE_ID = "
	 * +featureId +
	 * " LEFT JOIN SPATIAL_FEATURES AS SF ON FM.FEATURE_ID = SF.FEATURE_ID" +
	 ...
	 * 
	 * 
	 * List<Attribute> attribList = new ArrayList<Attribute>(); Cursor cursor =
	 * database.rawQuery(sql, null);
	 * 
	 * if (cursor.moveToFirst()) { do { Attribute attrib = new Attribute();
	 * attrib.setAttributeid(cursor.getInt(1));
	 * attrib.setControlType(cursor.getInt(3)); if(lang.equalsIgnoreCase("sw")
	 * && !TextUtils.isEmpty(cursor.getString(6))) {
	 * attrib.setAttributeName(cursor.getString(6)); } else {
	 * attrib.setAttributeName(cursor.getString(4)); }
	 * 
	 * 
	 * attrib.setValidation(cursor.getString(7));
	 ...
	 * if (attrib.getControlType()==5) {
	 * 
	 * Option option =null; List<Option> optionList = new ArrayList<Option>();
	 * option=new Option(); option.setOptionId(0L);
	 * option.setOptionName(contxt.getResources().getString(R.string.select));
	 * optionList.add(option);
	 * 
	 * String selectQueryOptions = "SELECT  * FROM OPTIONS where ATTRIB_ID ="+
	 * attrib.getAttributeid(); Cursor cursor2 =
	 * database.rawQuery(selectQueryOptions, null);
	 * 
	 * if (cursor2.moveToFirst()) { do { option = new Option();
	 * option.setOptionId(cursor2.getLong(0)); StringBuffer multiLnagText = new
	 * StringBuffer(); if(lang.equalsIgnoreCase("sw") &&
	 * !TextUtils.isEmpty(cursor2.getString(3))){
	 * option.setOptionName(cursor2.getString(3)); }else{
	 * option.setOptionName(cursor2.getString(2)); }
	 * if(!TextUtils.isEmpty(cursor2.getString(3))){
	 * multiLnagText.append(cursor2.getString(3));
	 * multiLnagText.append("&#&"+cursor2.getString(2));
	 * option.setOptionMultiLang(multiLnagText.toString()); }else{
	 * multiLnagText.append(cursor2.getString(2));
	 * multiLnagText.append("&#&"+cursor2.getString(2));
	 * option.setOptionMultiLang(multiLnagText.toString()); }
	 * optionList.add(option); } while (cursor2.moveToNext()); }cursor2.close();
	 * attrib.setOptionsList(optionList); } attribList.add(attrib); } while
	 * (cursor.moveToNext()); }cursor.close(); return attribList; }
	 */
	...
	public boolean isGeneralAttributeSaved(long featureId) {
		...
		String selectSQL = "SELECT PARCEL_TYPE FROM SPATIAL_FEATURES WHERE FEATURE_ID = "
				+ featureId;
	...
	public List<Attribute> getGneralAttributeData(Long featureId,
			String keyword, String lang) // for hardcoded controls
	{

		String sql = "SELECT * FROM SPATIAL_FEATURES WHERE FEATURE_ID="
				+ featureId;
		...
		if (cursor.moveToFirst()) {
			do {
				...
				attrib.setParcelType(cursor.getString(9));
	...
	public int getAdminCount(Long featureId) // for hardcoded controls
	{
		...
		String sql = "SELECT * FROM PERSON WHERE PERSON_SUBTYPE='Administrator' and FEATURE_ID="
				+ featureId;
	...
	public int getOwnerCount(Long featureId) {
		...
		String sql = "SELECT * FROM PERSON WHERE PERSON_SUBTYPE='Owner' and FEATURE_ID="
				+ featureId;
	...
	public int getMinorCount(long featureId) {
		...
		String selectSQL = "SELECT * FROM FORM_VALUES WHERE FEATURE_ID = "
				+ featureId + " AND ATTRIB_ID=21";
	...
	public boolean setResidentValue(String residentValue, long featureId) {
		...
		try {
			...
			// updating Features
			ContentValues value = new ContentValues();
			value.put("SERVER_PK", residentValue); // TODO New coloumn is to be
													// added to save resident
													// value instead of saving
													// it in server pk coloumn
			db.update("SPATIAL_FEATURES", value, whereClause, null);
	...
	public String getResidentValue(long featureId) {
		...
		String selectSQL = "SELECT SERVER_PK FROM SPATIAL_FEATURES WHERE FEATURE_ID = "
				+ featureId; // currently SERVER_PK is used as Resident coloumn
	...
	public boolean isOwnerOrGuardianImageExist(long featureId, int groupId,
			String personSubtype) {
		SQLiteDatabase database = getReadableDatabase();
		boolean flag = false;

		String selectSQL = "SELECT PER_MEDIA.* FROM PERSON_MEDIA AS PER_MEDIA LEFT OUTER JOIN PERSON AS PER ON PER_MEDIA.PERSON_ID=PER.ID where PER.ID = "
				+ groupId
				+ " AND PER.PERSON_SUBTYPE= '"
				+ personSubtype
				+ "' AND PER.FEATURE_ID= " + featureId;
	...
	public long getOwnerAndGuardianCount(long featureId) {
		...
		SQLiteStatement s = database
				.compileStatement("select count(*) from PERSON where FEATURE_ID='"
						+ featureId + "'; ");
	...
	/* CHANGES FOR BURKINO FASO */

	public boolean saveAttributeData(List<Attribute> attribList, int groupid,
			long featureId, String keyword) {
		ContentValues values = new ContentValues();
		List<ContentValues> valueList = new ArrayList<ContentValues>();
		SQLiteDatabase db = getWritableDatabase();
		...
		if (groupid != 0) {
			try {
               ...
               tableValues.put("TENURE_TYPE_ID",attribute.getFieldValue());
						}
					}
				}
				...
				if (keyword.equalsIgnoreCase("PERSON")) {
					...
					tableValues.remove("ATTRIB_1");
					tableValues.remove("ATTRIB_2");
					db.insert("SOCIAL_TENURE", null, tableValues);
				}
	...
	public String getParcelType(long featureId) {
		SQLiteDatabase database = getReadableDatabase();
		String residentValue = "Select an option";
	...
	public int getTenureTypeId(long featureId) {
		SQLiteDatabase database = getReadableDatabase();
		int tenureType = 0;

		String selectSQL = "SELECT TENURE_TYPE_ID FROM SOCIAL_TENURE WHERE FEATURE_ID = "
				+ featureId;
	...
	public List<Attribute> getPOIAttribute(SQLiteDatabase database, String lang) {
	...
	public List<Attribute> getPoiList(Long featureId) {
		SQLiteDatabase database = getReadableDatabase();
		List<Attribute> attribList = new ArrayList<Attribute>();
		String selectQueryQues = "SELECT * from POI where FEATURE_ID ="
				+ featureId;
		...
		if (cursor.moveToFirst()) {
			do {
				...
				/*String personSubType = cursor.getString(5);
				String fieldValue = "";
				if (!TextUtils.isEmpty(personSubType)) {} else {
					fieldValue = value1 + " " + value2;
				}*/
				String fieldValue = "";
				fieldValue = value1 + " " + value2;
	...
	public int getTenureCount(long featureId) {
		SQLiteDatabase database = getReadableDatabase();
		int tenureCount = 0;

	
		String selectSQL = "SELECT * from SOCIAL_TENURE AS ST left join PERSON AS P where ST.FEATURE_ID ="
				+ featureId;
	...
	public boolean saveOtherExistingUse( Long featureId,String otherUse) {
		String whereClause = "FEATURE_ID = " + featureId;
		try {
			SQLiteDatabase db = getWritableDatabase();
			// updating Features
			ContentValues value = new ContentValues();
			value.put("OTHER_EXISTING_USE", otherUse);
	...
	public String getExistingOtherUse(Long featureId) {
		String existingUse =null;

		SQLiteDatabase database = getReadableDatabase();

		String selectSQL = "SELECT OTHER_EXISTING_USE FROM SPATIAL_FEATURES WHERE FEATURE_ID = "+featureId;
		Cursor cursor = database.rawQuery(selectSQL, null);

		if (cursor.moveToFirst()) {
			existingUse = cursor.getString(0);
		}
		cursor.close();
	...
	public boolean saveDateOfMandate( Long featureId,String date) {
		String whereClause = "FEATURE_ID = " + featureId;
		try {
			SQLiteDatabase db = getWritableDatabase();
			// updating Features
			ContentValues value = new ContentValues();
			value.put("PERSON_SUBTYPE", date);

			db.update("PERSON", value, whereClause, null);
	 ...
	 public void reopen() throws SQLException {
		    close();
		    com.rmsi.android.mast.db.DBController dbOpenHelper = new com.rmsi.android.mast.db.DBController(contxt);
